---
name: sdkwork-backend-go-integration
description: Use when integrating the sdkwork-backend-sdk-go package into an application and you need staged progressive loading with explicit stage gates.
---

# Scope

Integrate `sdkwork-backend-sdk-go` into an application using progressive stages. Advance only when the current stage exits green.
...
- Local-path integration: `go mod edit -replace github.com/sdkwork/backend-sdk=<path-to-sdkwork-backend-sdk-go> && go mod tidy`
