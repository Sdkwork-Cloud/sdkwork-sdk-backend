---
name: sdkwork-backend-java-integration
description: Use when integrating the sdkwork-backend-sdk-java package into an application and you need staged progressive loading with explicit stage gates.
---

# Scope

Integrate `sdkwork-backend-sdk-java` into an application using progressive stages. Advance only when the current stage exits green.
...
- Registry integration: `mvn dependency:get -Dartifact=com.sdkwork:backend-sdk:1.0.0`
- Local-path integration: `mvn install:install-file -Dfile=<path-to-jar> -DgroupId=com.sdkwork -DartifactId=backend-sdk -Dversion=1.0.0 -Dpackaging=jar`
