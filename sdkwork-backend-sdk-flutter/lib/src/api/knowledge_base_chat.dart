...
class KnowledgeBaseChatApi {
  ...
  Future<PlusApiResultBoolean?> stop(Map<String, String>? headers) async {
    final response = await _client.post(ApiPaths.backendPath('/knowledge_base/chat/stop'), headers: headers);
  ...
  Future<ChatCompletionChunk?> create(ChatCompletionCreateForm body, Map<String, String>? headers) async {
    final response = await _client.post(ApiPaths.backendPath('/knowledge_base/chat/completions'), body: body, headers: headers, contentType: 'application/json');
