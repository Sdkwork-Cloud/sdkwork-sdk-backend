...
class DatasourceApi:
    ...
    def stop(self, headers: Optional[Dict[str, str]] = None) -> PlusApiResultBoolean:
        return self._client.post(f"/backend/v3/api/datasource/stop", headers=headers)
    ...
    def create_completions(self, body: ChatCompletionCreateForm, headers: Optional[Dict[str, str]] = None) -> ChatCompletionChunk:
        """Create a chat completion with Datasource"""
        return self._client.post(f"/backend/v3/api/datasource/chat/completions", json=body, headers=headers)
