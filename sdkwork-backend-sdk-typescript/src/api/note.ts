...
/** Query publish status */
  async queryMediaPublishStatus(body: PlusNotesMediaPublishStatusForm): Promise<PlusApiResultPlusMediaPublishResultDTO> {
    return this.client.post<PlusApiResultPlusMediaPublishResultDTO>(backendApiPath(`/notes/media/publish/status`), body);
  }

/** Query published article */
  async queryPublishedArticle(body: PlusNotesMediaPublishedArticleForm): Promise<PlusApiResultPlusMediaPublishResultDTO> {
    return this.client.post<PlusApiResultPlusMediaPublishResultDTO>(backendApiPath(`/notes/media/publish/article`), body);
  }
...
export function createNoteApi(client: HttpClient): NoteApi {
  return new NoteApi(client);
}
