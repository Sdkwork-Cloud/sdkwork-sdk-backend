...
export interface PlusConversationVO {
  ...
  /** Conversation title */
  title?: string;
  /** Conversation type (1: Chat, 2: C2C, 3: Group, 99: KF) */
  type?: 'CHAT' | 'C2C' | 'GROUP' | 'KF';
  /** Conversation description (optional) */
  description?: string;
  /** User identifier (foreign key to plus_user.id) */
  userId?: number;
  /** Conversation status (1: Active, 2: Completed, 3: Archived, 4: Deleted) */
  ...
  /** Chat scene */
  scene?: string;
  /** Content summary (brief overview of the conversation) */
  summary?: string;
  /** Last message ID (foreign key to plus_chat_message.id) */
  lastMessageId?: number;
  /** Message count in this conversation */
  messageCount?: number;
  /** Unread message count */
  unreadCount?: number;
  /** Conversation tags (comma-separated, e.g., "important,work,project") */
  tags?: TagsContent;
  /** System context/parameters for the conversation (JSON format) */
  systemContext?: string;
  /** User context/parameters for the conversation (JSON format) */
  userContext?: string;
  ...
  /** Data source ID (foreign key to plus_data_source_info.id) */
  dataSourceId?: number;
  /** Chat options (temperature, top_p, etc.) stored as JSON */
  chatOptions?: PlusChatOptions;
  /** Whether the conversation is pinned to the top of the conversation list */
  pinned?: boolean;
  /** The order of conversations, higher values appear first */
  sortOrder?: number;
}
